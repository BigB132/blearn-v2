...
const getHomeworks = async (req, res) => {
  ...
  if (!username || !password) {
    return res
      .status(400)
      .json({ state: "error", message: "Missing username or password" });
  }
...
const createHomework = async (req, res) => {
  ...
  if (!username || !password || !subjectId || !dueDate || !description) {
    return res
      .status(400)
      .json({ state: "error", message: "Missing required fields" });
  }
  ...
  res
    .status(200)
    .json({ state: "success", message: "Homework created successfully" });
};
...
const completeHomework = async (req, res) => {
  ...
  if (!homework) {
    return res
      .status(404)
      .json({ state: "error", message: "Homework not found" });
  }
  ...
  res
    .status(200)
    .json({ state: "success", message: "Homework updated successfully" });
};

const editHomework = async (req, res) => {
  const { username, password, homeworkId, subjectId, dueDate, description } =
    req.body;
  ...
  res
    .status(200)
    .json({ state: "success", message: "Successfully edited homework!" });
};
...
const deleteHomework = async (req, res) => {
  ...
  console.log(
    user.userName +
      " has deleted his homework with id " +
      homeworkId +
      " successfully."
  );
